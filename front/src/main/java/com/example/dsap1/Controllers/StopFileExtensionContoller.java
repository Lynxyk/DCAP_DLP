...
@Component
@Lazy
public class StopFileExtensionContoller {
    ...
    void initialize() {
        ...
        inpCoiceFile.setPromptText("Введите новое расширение файла");

        // Подтягиваем существующие стоп-слова
        loadStopExtFromBackend();
     ...
     */
    private void loadStopExtFromBackend() {
        try {
            StopExtension[] stopExtensions = restTemplate.getForObject(baseUrl + "/extensions", StopExtension[].class);
            if (stopExtensions == null) return;

            // Преобразуем в список
            List<StopExtension> list = new ArrayList<>(Arrays.asList(stopExtensions));

            // Сортируем: если role=null, считаем её как пустую строку
            list.sort(Comparator.comparing(se ->
                    se.getRole() != null ? se.getRole().getName() : ""
            ));

            // Теперь добавляем в интерфейс в нужном порядке
            for (StopExtension se : list) {
                addStopExtToUI(se);
            }
        } catch (Exception e) {
            e.printStackTrace();
     ...
     */
    private void addStopExtToUI(StopExtension se) {
        // Горизонтальный контейнер
        HBox row = new HBox(10); // 10 – отступы между элементами
        row.setAlignment(Pos.CENTER_LEFT);

        // Текстовое поле со словом
        TextField textField = new TextField(se.getExtension());
        ...
        TextField roleField = new TextField(
                se.getRole() != null ? se.getRole().getName() : "Без роли"
        );
    ...
    private void deleteStopExt(Long id) {
        try {
            restTemplate.delete(baseUrl + "/extensions" + "/" + id);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    void addGo(ActionEvent event) {
        ...
        StopExtension newExtension = new StopExtension();
        newExtension.setExtension(value);

        // Берём выбранную роль из ComboBox, если есть
        Role selectedRole = comboBoxRole.getValue();
        if (selectedRole != null) {
            newExtension.setRole(selectedRole);
        }

        try {
            if (comboBoxRole.getValue() != null){
                // Отправляем POST на бэкенд
                StopExtension created = restTemplate.postForObject(baseUrl + "/extensions", newExtension, StopExtension.class);
                if (created != null) {
                    addStopExtToUI(created);
                }
                // Очищаем поле
                inpCoiceFile.clear();
            }
